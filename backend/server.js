...
app.post("/login", (req, res) =>{
    ...
    db.query(sql, [email], async(err, results) =>{
        if(err) return res.status(500).json({messege: "Server Error"});
        if(results.length === 0){
        ...
        db.query(sql, [first_name, last_name, email, hashedPassword], (err, results) =>{
            if(err){
                if(err.code === "ER_DUP_ENTRY"){
                    return res.status(400).json({messege: "Email Already Registered"});
                }
                return res.status(500).json({messege: "Server Error"});
            }
            res.status(201).json({messege: "User Registered Successfully", userId: result.insertId});
        });
    }catch(err){
